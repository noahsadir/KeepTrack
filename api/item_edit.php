...
$remind_before = $params->remind_before;
...
if (mysqli_connect_errno()){
  ...
  if (canEditList($connection, $internal_id, $list_id)) {
    if (itemExists($connection, $item_id)) {
      editListItem($connection, $internal_id, $list_id, $item_id, $title, $notes, $tag, $start_date, $end_date, $duration, $repeat_interval, $remind_before, $priority);
    } else {
      http_response_code(400);
      header('Content-Type: application/json');
...
function editListItem($connection, $internal_id, $list_id, $item_id, $title, $notes, $tag, $start_date, $end_date, $duration, $repeat_interval, $remind_before, $priority) {
  $unique_id = generateRandomToken(16);
  if ($statement = $connection->prepare('UPDATE listitems SET title = ?, notes = ?, tag = ?, start_date = ?, end_date = ?, repeat_interval = ?, remind_before = ?, duration = ?, priority = ?, last_modified = ? WHERE (item_id = ? AND list_id = ?)')) {
    $statement->bind_param('sssiiiiiiiss', $title, $notes, $tag, $start_date, $end_date, $repeat_interval, $remind_before, $duration, $priority, round(microtime(true) * 1000), $item_id, $list_id);
    $statement->execute();
    $statement->store_result();

    http_response_code(200);
    header('Content-Type: application/json');
    outputJSON('{"success": true, "message": "Successfully edited item."}');
  } else {
    http_response_code(500);
    header('Content-Type: application/json');
